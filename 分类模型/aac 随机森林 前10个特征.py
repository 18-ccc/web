...
# 加载新的 AAC 分析结果文件
file_path = r'D:\bishedata\baomancdhit+阴性样本 aac.xlsx'
...
# 选择前10个特征
X_top10 = X[top10_features]  # 只保留前10个特征

# 数据标准化：只对前10个特征进行标准化
X_top10_scaled = scaler.fit_transform(X_top10)

# 划分训练集和测试集
X_train, X_test, y_train, y_test = train_test_split(X_top10_scaled, y, test_size=0.3, random_state=42)
...
grid_search_balanced = GridSearchCV(RandomForestClassifier(class_weight='balanced'), param_grid, cv=10, scoring='accuracy', n_jobs=-1, verbose=2)
