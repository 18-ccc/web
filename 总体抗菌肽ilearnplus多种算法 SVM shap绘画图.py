...
# 从测试集中随机采样 50 个样本
sample_idx = np.random.choice(X_test_df.shape[0], 50, replace=False)
X_test_sample = X_test_df.iloc[sample_idx]

# 计算 SHAP 值（注意是对正类进行解释）
shap_values = explainer.shap_values(X_test_sample)

# ========== 10. SHAP 重要性条形图 ==========
shap.summary_plot(
    shap_values, X_test_sample, plot_type="bar",
    max_display=10, show=False
)
plt.title("SHAP特征重要性条形图（Top 10）", fontsize=16)
plt.tight_layout()
plt.show()

# ========== 11. SHAP 蜂群图（Beeswarm） ==========
shap.summary_plot(
    shap_values, X_test_sample, plot_type="dot",
