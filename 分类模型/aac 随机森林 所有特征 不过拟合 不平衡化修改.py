...
X_train_scaled = scaler.fit_transform(X_train)  # 使用训练集的均值和标准差拟合并转换
X_test_scaled = scaler.transform(X_test)  # 使用训练集的标准化参数对测试集进行标准化
...
# 使用网格搜索和交叉验证
grid_search = GridSearchCV(RandomForestClassifier(), param_grid, cv=10, scoring='accuracy', n_jobs=-1, verbose=2)
